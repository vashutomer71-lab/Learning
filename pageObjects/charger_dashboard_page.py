...
xpath_charger_menu = "(//div/span[contains(text(),'Chargers')])[1]"
xpath_charger_text = "//h2[normalize-space(text()) = 'Chargers']"
xpath_charger_tile = "(//div[@class='agr_data_box'])[1]"
xpath_port_tile = "(//div[@class='agr_data_box'])[2]"
xpath_charger_list_heading = "//div[@class='MuiGrid-root MuiGrid-item MuiGrid-grid-xs-12 MuiGrid-grid-sm-6 MuiGrid-grid-md-6 MuiGrid-grid-lg-8 css-32ukjs']"
...
class ChargerDashboardPage:
    ...
    def click_charger_menu(self):
        self.page.wait_for_selector(xpath_charger_menu).click()

    def verify_charger_dashboard_heading(self):
        expected = "CHARGERS"
        fleet_text_frontend = self.page.inner_text(xpath_charger_text)
        # Improved print message for clarity
        print(f"Verifying charger Dashboard Heading...\nExpected: '{expected}'\nActual: '{fleet_text_frontend}'")
    ...
    def verify_chargers_tile(self, fetch_data):
        ...
        print("tile text::", charger_tile_text_count)
    ...
    def verify_port_tile(self, fetch_data):
        self.page.wait_for_timeout(3000)
        port_tile_text_count = self.page.inner_text(xpath_port_tile).splitlines()   # Splitting the text by lines to extract each piece of data
        # Extract individual components from the text
        print("port tile text::", port_tile_text_count)
        total_port_count = int(port_tile_text_count[0])  # Assuming the total count is the first line

        correct yaha tak

        charger_text = port_tile_text_count[1]  # Should say "Chargers"
        available_count = int(port_tile_text_count[2])  # Count for Available
        available_text = port_tile_text_count[3]  # Should say "Available"
        busy_count = int(port_tile_text_count[4])  # Count for Busy
        busy_text = port_tile_text_count[5]  # Should say "Busy"
        offline_count = int(port_tile_text_count[6])  # Count for Offline
        offline_text = port_tile_text_count[7]  # Should say "Offline"
        faulty_count = int(port_tile_text_count[-3])  # Count for Faulty (assuming it's the third-to-last line)
        faulty_text = port_tile_text_count[-2]  # Should say "Faulty"
        # Execute SQL query to fetch data from the database
        charger_tile_query = "charger_tile_count_query"
        headers, result = fetch_data(charger_tile_query)
        db_table_view= tabulate(result, headers=headers, tablefmt="grid")   # Display the database results in a tabular format
